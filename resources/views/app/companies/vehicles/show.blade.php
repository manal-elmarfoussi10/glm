    ...
    <div class="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] shadow-xl">
        <div class="aspect-video w-full overflow-hidden bg-slate-800/50">
            @if ($vehicle->image_url)
                <img src="{{ $vehicle->image_url }}" alt="{{ $vehicle->plate }}" class="h-full w-full object-cover transition duration-300 hover:scale-[1.02]" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
    ...
    <div x-show="tab === 'documents'" x-cloak class="glm-card-static p-6 rounded-xl">
        ...
        @if ($vehicle->insurance_document_path || $vehicle->vignette_receipt_path || $vehicle->visite_document_path || $vehicle->financing_contract_path)
            <div class="flex flex-wrap gap-3">
                @if ($vehicle->insurance_document_path)<a href="{{ storage_public_url($vehicle->insurance_document_path) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white hover:bg-white/10 transition no-underline"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Assurance</a>@endif
                @if ($vehicle->vignette_receipt_path)<a href="{{ storage_public_url($vehicle->vignette_receipt_path) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white hover:bg-white/10 transition no-underline"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Reçu vignette</a>@endif
                @if ($vehicle->visite_document_path)<a href="{{ storage_public_url($vehicle->visite_document_path) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white hover:bg-white/10 transition no-underline"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>Visite technique</a>@endif
                @if ($vehicle->financing_contract_path)<a href="{{ storage_public_url($vehicle->financing_contract_path) }}" target="_blank" class="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white hover:bg-white/10 transition no-underline"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Contrat financement</a>@endif
            </div>
        @else
