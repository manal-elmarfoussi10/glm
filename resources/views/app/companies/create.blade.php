...
<div class="space-y-8 glm-fade-in">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <a href="{{ route('app.companies.index') }}" class="text-sm font-medium text-slate-400 hover:text-white mb-2 inline-block transition-colors">← Retour à la liste</a>
            <h1 class="text-2xl font-bold tracking-tight text-white">Nouvelle entreprise</h1>
            <p class="mt-1 text-sm text-slate-400">Renseignez les informations de l’entreprise.</p>
        </div>
    </div>

    <form action="{{ route('app.companies.store') }}" method="post" class="glm-card-static p-6 max-w-2xl space-y-6">
        @csrf
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            <div class="sm:col-span-2">
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50" placeholder="Ex. Atlas Rent Cars">
                @error('name')<p class="mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="ice" class="mb-1.5 block text-sm font-medium text-slate-300">ICE</label>
                <input type="text" id="ice" name="ice" value="{{ old('ice') }}" class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50" placeholder="Identifiant commun entreprise">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50" placeholder="contact@entreprise.ma">
                @error('email')<p class="mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="city" class="mb-1.5 block text-sm font-medium text-slate-300">Ville</label>
                <input type="text" id="city" name="city" value="{{ old('city') }}" class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50" placeholder="Casablanca">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="address" name="address" rows="2" class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50" placeholder="Adresse complète">{{ old('address') }}</textarea>
        ...
        <div class="flex gap-3 pt-2">
            <a href="{{ route('app.companies.index') }}" class="glm-btn-secondary no-underline">Annuler</a>
            <button type="submit" class="glm-btn-primary">Créer l’entreprise</button>
        </div>
    </form>
</div>
@endsection
