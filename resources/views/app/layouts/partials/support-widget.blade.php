...
@endphp
<div class="fixed bottom-6 right-6 z-40" x-data="{ supportOpen: false }">
    {{-- Popup panel --}}
    <div x-show="supportOpen" x-cloak
         class="absolute bottom-14 right-0 w-[360px] max-w-[calc(100vw-3rem)] glm-card-static border border-white/10 shadow-xl overflow-hidden"
        ...
        <div class="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            @elseif ($isPlatformStaff)
                <p class="text-sm text-slate-400">Gérez les tickets depuis l'inbox.</p>
                <a href="{{ route('app.inbox.index') }}" class="block w-full text-center glm-btn-primary text-sm py-2 no-underline">Ouvrir l'inbox</a>
            @else
    ...
    <button type="button" @click="supportOpen = !supportOpen"
            class="flex h-12 w-12 items-center justify-center rounded-full bg-[#2563EB] text-white shadow-lg hover:bg-[#1d4ed8] transition focus:outline-none focus:ring-2 focus:ring-white/30"
