...
<div class="space-y-8 glm-fade-in" x-data="contractTemplateForm({{ json_encode([
    ...
    <form action="{{ route('app.admin.contract-templates.update', $template) }}" method="post" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="grid gap-6 lg:grid-cols-[1fr_280px]">
            <div class="glm-card-static p-6 space-y-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $template->name) }}" required class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50">
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="18" class="w-full rounded-xl border-0 bg-white/5 px-4 py-2.5 text-sm text-white font-mono placeholder-slate-500 focus:ring-2 focus:ring-[#2563EB]/50">{{ old('content', $template->content) }}</textarea>
